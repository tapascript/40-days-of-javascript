...
let currentUser = new Member('John Doe', 'john@example.com');
...
function renderBorrowed() {
  ...
  borrowedBooks.forEach(book => {
    const li = document.createElement('li');
    li.className = 'bg-yellow-100 p-4 rounded shadow flex justify-between items-center';
    li.innerHTML = `
      <div>
...
borrowedList.addEventListener('click', e => {
  if (e.target.tagName === 'BUTTON') {
    const id = e.target.getAttribute('data-id');
    const book = library.getBookById(id);
    if (currentUser.getRole() === 'Member' && book && !book.isAvailable) {
...
userSwitcher.addEventListener('change', e => {
  ...
  currentUser = selected === 'admin' ? new Admin('Alice', 'admin@example.com') : new Member('John Doe', 'john@example.com');
  bookForm.parentElement.style.display = selected === 'admin' ? 'block' : 'none';
